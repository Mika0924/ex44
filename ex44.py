...
def copy_data(source_file, destination_file, row_number):
    ...
    if 0 < row_number <= len(data):
        with open(destination_file, 'a', encoding='utf-8', newline='') as dest_data:
            f_writer = DictWriter(dest_data, ["Имя", "Фамилия", "Телефон"])
            f_writer.writerow(data[row_number - 1])
...
file_name = 'phone.csv'
backup_file_name = 'phone_backup.csv'
...
def main():
    while True:
        ...
        if command == 'q':
            ...
            row_num = int(input("Введите номер строки для копирования: "))
            copy_data(file_name, backup_file_name, row_num)
